...
type CrewRole =
  ...
  | 'On Board'
  | 'PIC';
type CrewRoleFilter = CrewRole | '全部';
...
const crewRoleOptions: { label: string; value: CrewRoleFilter }[] = [
  { label: '全部职责', value: '全部' },
  { label: '责任机长（PIC）', value: 'PIC' },
  { label: '机长（Captain）', value: 'Captain' },
  { label: '副驾驶（F/O）', value: 'F/O' },
  { label: '飞行教员', value: 'Flight Instructor' },
  { label: '乘务员', value: 'Flight attendant' },
  { label: '乘务教员', value: 'Flight attendant Instructor' },
  { label: '托管乘务员', value: 'F/A(Private)' },
  { label: '维修工程师', value: 'Mechanic' },
  { label: '随机人员', value: 'On Board' },
  { label: '飞行检查员', value: 'Inspector' },
];

const crewRoleLabels = Object.fromEntries(
  crewRoleOptions
    .filter((option): option is { label: string; value: CrewRole } => option.value !== '全部')
    .map((option) => [option.value, option.label.replace(/（.*）/, '')]),
...
const crewMembers: CrewMember[] = [
  ...
  { base: 'ZSPD', dutyHours: '06:20 / 13:00', id: 'CREW-A-SIC', name: '李睿', qualification: 'G650ER · G450', readiness: 91, role: 'F/O', state: 'ready', stateLabel: '可用', team: 'A 组' },
  { base: 'ZSPD', dutyHours: '06:20 / 13:00', id: 'CREW-A-CC', name: '许静', qualification: '国际航线', readiness: 94, role: 'Flight attendant', state: 'ready', stateLabel: '可用', team: 'A 组' },
  ...
  { base: 'ZBAA', dutyHours: '09:40 / 13:00', id: 'CREW-B-SIC', name: '陈昊', qualification: 'G650ER · G550', readiness: 77, role: 'F/O', rosterIssue: '前序执勤休息不足', state: 'warning', stateLabel: '需关注', team: 'B 组' },
  { base: 'ZBAA', dutyHours: '09:40 / 13:00', id: 'CREW-B-CC', name: '周宁', qualification: '国际航线', readiness: 82, role: 'Flight attendant', state: 'ready', stateLabel: '可用', team: 'B 组' },
  ...
  { base: 'ZGGG', dutyHours: '11:55 / 13:00', id: 'CREW-C-SIC', name: '高宇', qualification: 'G550', readiness: 46, role: 'F/O', rosterIssue: '与 B-801Q 航班冲突', state: 'conflict', stateLabel: '受阻', team: 'C 组' },
  { base: 'ZGGG', dutyHours: '11:55 / 13:00', id: 'CREW-C-CC', name: '林安', qualification: '国内航线', readiness: 52, role: 'Flight attendant', state: 'warning', stateLabel: '需关注', team: 'C 组' },
  ...
  { base: 'ZSPD', dutyHours: '00:00 / 13:00', id: 'CREW-D-SIC', name: '沈乔', qualification: 'G650ER · G450', readiness: 100, role: 'F/O', state: 'rest', stateLabel: '休息中', team: 'D 组' },
  { base: 'ZSPD', dutyHours: '00:00 / 13:00', id: 'CREW-D-CC', name: '杨希', qualification: '国际航线', readiness: 100, role: 'Flight attendant', rosterIssue: '休假至 8月24日', state: 'rest', stateLabel: '休息中', team: 'D 组' },
  { base: 'ZSPD', dutyHours: '04:30 / 13:00', id: 'CREW-E-INSTRUCTOR', name: '唐峻', qualification: 'G650ER · 飞行教员', readiness: 96, role: 'Flight Instructor', state: 'ready', stateLabel: '可用', team: '教员组' },
  { base: 'ZBAA', dutyHours: '05:10 / 13:00', id: 'CREW-E-CAPTAIN', name: '韩川', qualification: 'G450 · G550', readiness: 89, role: 'Captain', state: 'ready', stateLabel: '可用', team: 'E 组' },
  { base: 'ZSPD', dutyHours: '03:20 / 13:00', id: 'CREW-E-CC-INSTRUCTOR', name: '林悦', qualification: '国际航线 · 客舱教员', readiness: 93, role: 'Flight attendant Instructor', state: 'ready', stateLabel: '可用', team: '客舱教员组' },
  { base: 'ZBAA', dutyHours: '02:40 / 13:00', id: 'CREW-E-PRIVATE-CC', name: '苏妍', qualification: 'B-9811 托管机', readiness: 95, role: 'F/A(Private)', state: 'ready', stateLabel: '可用', team: '托管组' },
  { base: 'ZGGG', dutyHours: '06:00 / 13:00', id: 'CREW-E-MECHANIC', name: '郑工', qualification: 'G650ER · G450 维修放行', readiness: 90, role: 'Mechanic', state: 'ready', stateLabel: '可用', team: '机务组' },
  { base: 'ZSPD', dutyHours: '07:10 / 13:00', id: 'CREW-E-ONBOARD', name: '罗凯', qualification: '运行随行', readiness: 83, role: 'On Board', state: 'warning', stateLabel: '需关注', team: '运行组' },
  { base: 'ZBAA', dutyHours: '08:25 / 13:00', id: 'CREW-E-INSPECTOR', name: '蒋宁', qualification: '飞行运行检查', readiness: 68, role: 'Inspector', rosterIssue: '检查授权待确认', state: 'warning', stateLabel: '需关注', team: '检查组' },
];
...
const onlyAvailableRosterCrew = ref(false);
...
const visibleCrew = computed(() => crewMembers.filter((member) =>
  (!crewSearch.value.trim() || `${member.name}${member.role}${crewRoleLabel(member.role)}${member.qualification}`.toLowerCase().includes(crewSearch.value.trim().toLowerCase())) &&
...
const sicOptions = computed(() => crewMembers.filter((member) => member.role === 'F/O'));
const cabinOptions = computed(() => crewMembers.filter((member) => member.role === 'Flight attendant'));
...
const rosterCrewCandidates = computed(() => crewMembers.filter((member) =>
  ...
  (!onlyAvailableRosterCrew.value || !isRosterCrewUnavailable(member)) &&
  (!rosterCrewSearch.value.trim() || `${member.name}${member.role}${crewRoleLabel(member.role)}${member.qualification}${member.base}`.toLowerCase().includes(rosterCrewSearch.value.trim().toLowerCase())),
));
...
const rosterRolesComplete = computed(() => ['PIC', 'F/O', 'Flight attendant'].every((role) => selectedRosterCrew.value.some((member) => member.role === role)));
...
watch(onlyAvailableRosterCrew, (enabled) => {
  if (!enabled) return;
  selectedRosterCrewIds.value = selectedRosterCrewIds.value.filter((id) => {
    const member = crewMembers.find((candidate) => candidate.id === id);
    return member && !isRosterCrewUnavailable(member);
  });
});
...
function openAssignmentWorkspace() {
  ...
  onlyAvailableRosterCrew.value = false;
}
    ...
    <section v-if="assignmentWorkspaceOpen" class="assignment-workspace" aria-label="新建机组排班">
      ...
      <div v-if="rosterTargets.length" class="assignment-body">
        <aside class="target-panel">
          <header><div class="unit-switch" aria-label="排班单位" role="group"><button :class="{ active: rosterUnit === 'trip' }" type="button" @click="rosterUnit = 'trip'">按行程</button><button :class="{ active: rosterUnit === 'flight' }" type="button" @click="rosterUnit = 'flight'">按航班</button></div><strong>{{ selectedRosterFlights.length }}/{{ unassignedFlights.length }}</strong></header>
          <template v-if="rosterUnit === 'trip'">
            <div v-for="target in rosterTargets" :key="target.id" :class="{ partial: isTripPartiallySelected(target), selected: isTripSelected(target) }" class="target-option target-trip">
              <label class="target-summary">
                ...
                <span class="trip-summary-meta"><b>行程</b><small v-if="target.customer">客户 {{ target.customer }}</small><small class="leg-count">{{ target.flights.length }} 个航段</small></span>
        ...
        <aside class="assignment-crew-panel">
          <div class="crew-role-filter">
            <span>机组职责</span>
            <div aria-label="机组职责筛选" role="group">
              <button v-for="option in crewRoleOptions" :key="option.value" :class="{ active: rosterCrewRoleFilter === option.value }" type="button" @click="rosterCrewRoleFilter = option.value">{{ option.label }}</button>
            </div>
          </div>
          <div class="roster-crew-toolbar">
            <div class="roster-crew-search"><input v-model="rosterCrewSearch" aria-label="搜索待分配人员" placeholder="搜索姓名、岗位、资质或基地" type="search" /></div>
            <label class="available-crew-toggle">
              <input v-model="onlyAvailableRosterCrew" type="checkbox" />
              <i aria-hidden="true"><b></b></i>
              <strong>仅展示可用人员</strong>
            </label>
          </div>
          <div class="roster-crew-options">
            <label v-for="member in rosterCrewCandidates" :key="member.id" :class="[member.state, { incompatible: isRosterCrewUnavailable(member), selected: selectedRosterCrewIds.includes(member.id) }]" :title="member.rosterIssue || member.stateLabel">
              <input v-model="selectedRosterCrewIds" :disabled="isRosterCrewUnavailable(member)" :value="member.id" type="checkbox" />
              <b>{{ crewRoleLabel(member.role) }}</b><strong>{{ member.name }}</strong><span class="technical-grade">{{ member.qualification }}</span><em><i></i>{{ member.stateLabel }}</em>
            </label>
...
.assignment-workspace { isolation: isolate; display: grid; min-height: 0; flex: 1; grid-template-rows: auto minmax(0, 1fr); overflow: hidden; background: var(--sj-canvas); }
.assignment-header { display: grid; min-height: 58px; padding: var(--sj-space-3) var(--sj-space-5); align-items: center; grid-template-columns: 1fr var(--sj-control-default); gap: var(--sj-space-4); border-bottom: 1px solid var(--sj-border); background: var(--sj-surface-2); }.assignment-header > div:first-child { display: grid; }.assignment-header > div:first-child strong { font-size: 16px; }.unit-switch { display: inline-flex; padding: 2px; border: 1px solid var(--sj-border-strong); border-radius: var(--sj-radius-control); background: var(--sj-surface-1); }.unit-switch button { min-height: var(--sj-control-dense); padding: 0 var(--sj-space-4); border: 0; border-radius: var(--sj-radius-tag); color: var(--sj-text-2); background: transparent; font-size: 10px; font-weight: 700; }.unit-switch button.active { color: var(--sj-text-1); background: var(--sj-blue-soft); box-shadow: inset 0 -2px var(--sj-blue); }.workspace-close { display: grid; width: var(--sj-control-default); height: var(--sj-control-default); place-items: center; border: 1px solid var(--sj-border-strong); border-radius: var(--sj-radius-control); color: var(--sj-text-2); background: var(--sj-surface-1); }.workspace-close svg { width: 15px; }
.assignment-body { display: grid; min-height: 0; overflow: hidden; grid-template-columns: minmax(300px, .72fr) minmax(520px, 1.48fr); }.assignment-body > * { min-width: 0; border-right: 1px solid var(--sj-border); }.target-panel { height: 100%; min-height: 0; overflow-y: auto; background: var(--sj-surface-1); }.target-panel > header { position: sticky; z-index: 2; top: 0; display: flex; min-height: 48px; padding: 0 var(--sj-space-4); align-items: center; justify-content: space-between; border-bottom: 1px solid var(--sj-border); background: var(--sj-surface-1); }.target-panel > header span { color: var(--sj-text-3); font-size: 9px; letter-spacing: .08em; }.target-panel > header strong { color: var(--sj-amber); font: 750 14px var(--sj-font-data); }.target-option { position: relative; display: grid; min-height: 110px; padding: var(--sj-space-3) var(--sj-space-4) var(--sj-space-3) 44px; gap: var(--sj-space-2); border-bottom: 1px solid var(--sj-border); color: var(--sj-text-1); cursor: pointer; background: transparent; }.target-option:hover, .target-option.selected { background: var(--sj-surface-3); }.target-option.selected { box-shadow: inset 3px 0 var(--sj-blue); }.target-option.partial { box-shadow: inset 3px 0 var(--sj-amber); }.target-option.target-trip { display: block; min-height: 0; padding: 0; cursor: default; }.target-summary { position: relative; display: grid; min-height: 78px; padding: var(--sj-space-3) var(--sj-space-4) var(--sj-space-3) 44px; gap: var(--sj-space-2); cursor: pointer; }.target-option > input, .target-summary > input { position: absolute; top: var(--sj-space-4); left: var(--sj-space-4); width: 16px; height: 16px; accent-color: var(--sj-blue); }.target-option > span, .target-summary > span { display: flex; align-items: center; justify-content: space-between; }.target-summary > .trip-summary-meta { justify-content: flex-start; gap: var(--sj-space-2); }.target-summary > .trip-summary-meta .leg-count { margin-left: auto; }.target-option span b, .target-summary span b { padding: 1px var(--sj-space-2); border: 1px solid var(--sj-border-strong); border-radius: var(--sj-radius-tag); color: var(--sj-blue); font: 750 8px var(--sj-font-data); }.target-option span small, .target-option > small, .target-summary span small, .target-summary > small { color: var(--sj-text-3); font-size: 9px; }.target-option > strong, .target-summary > strong { font: 750 13px var(--sj-font-data); }.target-option > em, .target-summary > em { color: var(--sj-text-2); font: 9px var(--sj-font-data); font-style: normal; }.target-flight-options { position: relative; margin-left: var(--sj-space-6); border-top: 1px solid var(--sj-border); border-left: 1px solid var(--sj-border-strong); background: var(--sj-canvas); }.target-flight-options label { display: grid; min-height: 52px; padding: var(--sj-space-2) var(--sj-space-3); align-items: center; grid-template-columns: 16px 1fr; gap: 2px var(--sj-space-2); border-bottom: 1px solid var(--sj-grid); cursor: pointer; }.target-flight-options label:last-child { border-bottom: 0; }.target-flight-options label:hover, .target-flight-options label.selected { background: var(--sj-blue-soft); }.target-flight-options input { width: 14px; height: 14px; accent-color: var(--sj-blue); }.target-flight-options span { display: flex; min-width: 0; align-items: center; gap: var(--sj-space-2); }.target-flight-options span b { flex: 0 0 auto; }.target-flight-options span strong { overflow: hidden; font: 750 10px var(--sj-font-data); text-overflow: ellipsis; white-space: nowrap; }.target-flight-options time { grid-column: 2; color: var(--sj-text-3); font: 8px var(--sj-font-data); }
.assignment-crew-panel { display: grid; min-height: 0; grid-template-rows: auto auto minmax(0, 1fr) auto auto; overflow: hidden; background: var(--sj-surface-1); }.assignment-crew-panel .crew-selection-grid { padding: var(--sj-space-4); border-bottom: 1px solid var(--sj-border); }.crew-preview { display: grid; padding: var(--sj-space-4); gap: var(--sj-space-2); }.crew-preview article { display: grid; min-height: 52px; padding: 0 var(--sj-space-3); align-items: center; grid-template-columns: 36px 1fr auto; gap: var(--sj-space-2); border: 1px solid var(--sj-border); background: var(--sj-surface-2); }.crew-preview article > b { color: var(--sj-blue); font: 750 8px var(--sj-font-data); }.crew-preview article > div { display: grid; gap: 2px; }.crew-preview article strong { font-size: 11px; }.crew-preview article span { color: var(--sj-text-3); font-size: 8px; }.crew-preview article em { display: inline-flex; align-items: center; gap: var(--sj-space-1); color: var(--sj-lime); font-size: 8px; font-style: normal; }.crew-preview article.warning em { color: var(--sj-amber); }.crew-preview article.conflict em { color: var(--sj-red); }.crew-preview article.rest em { color: var(--sj-blue); }.crew-preview article em i { width: 6px; height: 6px; border-radius: 50%; background: currentColor; }.validation-message { display: flex; margin: var(--sj-space-3) var(--sj-space-4); padding: var(--sj-space-3); align-items: flex-start; gap: var(--sj-space-2); border: 1px solid var(--sj-lime); color: var(--sj-lime); background: var(--sj-lime-soft); }.validation-message.blocked { border-color: var(--sj-red); color: var(--sj-red); background: var(--sj-red-soft); }.validation-message > svg { width: 14px; flex: 0 0 14px; }.validation-message > span { font: 800 12px var(--sj-font-data); }.validation-message > div { display: grid; gap: var(--sj-space-1); }.validation-message strong { font-size: 10px; }.validation-message small { color: var(--sj-text-2); font-size: 9px; }.assignment-footer { position: relative; z-index: 30; display: flex; min-height: 64px; padding: 0 var(--sj-space-4); align-items: center; justify-content: space-between; gap: var(--sj-space-4); border-top: 1px solid var(--sj-border); background: var(--sj-surface-2); box-shadow: 0 -8px 24px rgb(0 0 0 / 24%); }.selected-crew-summary { display: flex; min-width: 0; align-items: center; gap: var(--sj-space-3); }.selected-crew-summary strong { flex: 0 0 auto; color: var(--sj-text-2); font-size: 11px; }.selected-crew-summary strong b { color: var(--sj-blue); font: 800 20px var(--sj-font-data); }.selected-crew-summary > span { overflow: hidden; color: var(--sj-text-1); font-size: 11px; text-overflow: ellipsis; white-space: nowrap; }.assignment-footer-actions { display: flex; flex: 0 0 auto; gap: var(--sj-space-2); }.assignment-empty { display: grid; min-height: 420px; place-content: center; justify-items: center; gap: var(--sj-space-2); color: var(--sj-text-3); }.assignment-empty svg { width: 30px; color: var(--sj-blue); }.assignment-empty strong { color: var(--sj-text-1); font-size: 14px; }.assignment-empty span { font-size: 10px; }
...
.available-crew-toggle { display: inline-flex; align-items: center; gap: var(--sj-space-2); cursor: pointer; }.available-crew-toggle input { position: absolute; width: 1px; height: 1px; opacity: 0; }.available-crew-toggle > i { position: relative; width: 30px; height: 16px; flex: 0 0 auto; border: 1px solid var(--sj-border-strong); border-radius: 999px; background: var(--sj-surface-3); transition: background var(--sj-duration-fast); }.available-crew-toggle > i b { position: absolute; top: 2px; left: 2px; width: 10px; height: 10px; border-radius: 50%; background: var(--sj-text-3); transition: transform var(--sj-duration-fast), background var(--sj-duration-fast); }.available-crew-toggle input:checked + i { border-color: var(--sj-blue); background: var(--sj-blue-soft); }.available-crew-toggle input:checked + i b { background: var(--sj-blue); transform: translateX(14px); }.available-crew-toggle input:focus-visible + i { outline: 2px solid var(--sj-blue); outline-offset: 2px; }.available-crew-toggle > strong { color: var(--sj-text-2); font-size: 9px; white-space: nowrap; }
.crew-role-filter { display: grid; padding: var(--sj-space-3) var(--sj-space-4) 0; gap: var(--sj-space-2); }.crew-role-filter > span { color: var(--sj-text-3); font-size: 9px; }.crew-role-filter > div { display: flex; flex-wrap: wrap; gap: var(--sj-space-2); }.crew-role-filter button { min-height: var(--sj-control-dense); padding: 0 var(--sj-space-3); border: 1px solid var(--sj-border-strong); border-radius: var(--sj-radius-tag); color: var(--sj-text-2); background: var(--sj-canvas); font-size: 9px; font-weight: 700; }.crew-role-filter button:hover { color: var(--sj-text-1); background: var(--sj-surface-3); }.crew-role-filter button.active { border-color: var(--sj-blue); color: var(--sj-blue); background: var(--sj-blue-soft); }.roster-crew-toolbar { display: grid; padding: var(--sj-space-2) var(--sj-space-4) var(--sj-space-3); align-items: center; grid-template-columns: minmax(180px, 520px) auto; justify-content: space-between; gap: var(--sj-space-4); border-bottom: 1px solid var(--sj-border); }.roster-crew-search { min-width: 0; }.roster-crew-search input { width: 100%; height: var(--sj-control-default); padding: 0 var(--sj-space-3); border: 1px solid var(--sj-border-strong); border-radius: var(--sj-radius-control); color: var(--sj-text-1); background: var(--sj-canvas); font-size: 10px; }.roster-crew-options { min-height: 0; padding: var(--sj-space-2) var(--sj-space-4); overflow-y: auto; }.roster-crew-options label { display: grid; min-height: 48px; padding: var(--sj-space-2); align-items: center; grid-template-columns: 18px 82px minmax(80px, 1fr) minmax(150px, auto) 68px; gap: var(--sj-space-2); border-bottom: 1px solid var(--sj-border); cursor: pointer; }.roster-crew-options label:hover, .roster-crew-options label.selected { background: var(--sj-surface-3); }.roster-crew-options label > input { width: 14px; height: 14px; accent-color: var(--sj-blue); }.roster-crew-options label > b { color: var(--sj-blue); font: 750 8px var(--sj-font-data); }.roster-crew-options label > strong { font-size: 11px; }.roster-crew-options .technical-grade { overflow: hidden; color: var(--sj-text-2); font-size: 9px; text-align: right; text-overflow: ellipsis; white-space: nowrap; }.roster-crew-options label em { display: inline-flex; align-items: center; justify-content: flex-end; gap: var(--sj-space-1); color: var(--sj-lime); font-size: 8px; font-style: normal; white-space: nowrap; }.roster-crew-options label.warning em { color: var(--sj-amber); }.roster-crew-options label.conflict em { color: var(--sj-red); }.roster-crew-options label.rest em { color: var(--sj-blue); }.roster-crew-options label em i { width: 6px; height: 6px; border-radius: 50%; background: currentColor; }.roster-crew-options > p { margin: var(--sj-space-5); color: var(--sj-text-3); text-align: center; font-size: 9px; }.assignment-crew-panel .validation-message { margin-top: var(--sj-space-3); }
...
@media (max-width: 1439px) { .assignment-body { grid-template-columns: minmax(220px, .65fr) minmax(360px, 1.2fr) minmax(290px, .9fr); }.roster-workspace, .roster-workspace.has-inspector { grid-template-columns: 220px minmax(0, 1fr); }.open-flight-strip { grid-template-columns: 220px minmax(0, 1fr) 132px; overflow: hidden; }.open-flight-list { grid-auto-columns: minmax(240px, 300px); } }
...
@media (max-width: 699px) { .assignment-header { grid-template-columns: 1fr var(--sj-control-default); }.assignment-body { display: block; overflow-y: auto; }.target-panel { height: auto; max-height: 300px; overflow-y: auto; }.assignment-crew-panel { display: block; overflow: visible; }.target-detail > header, .assignment-crew-panel > header { padding-inline: var(--sj-space-3); }.leg-list { padding: var(--sj-space-3); }.leg-list article { grid-template-columns: 30px 1fr; }.assignment-crew-panel .roster-crew-options { max-height: 340px; }.leg-list article time { grid-column: 2; }.assignment-crew-panel .crew-selection-grid, .crew-preview { grid-template-columns: 1fr; }.assignment-footer { align-items: stretch; padding-block: var(--sj-space-3); flex-wrap: wrap; }.selected-crew-summary, .assignment-footer-actions { width: 100%; }.assignment-footer-actions button { flex: 1; }.roster-workspace { grid-template-columns: 150px minmax(620px, 1fr); }.crew-card { min-height: 82px; padding-inline: var(--sj-space-2); grid-template-columns: minmax(0, 1fr) auto; gap: var(--sj-space-1); }.crew-card > span { grid-column: 1 / -1; }.open-flight-strip { grid-template-columns: 150px minmax(0, 1fr) 112px; }.open-flight-action { padding: var(--sj-space-2); } }
/* New roster workspace: preserve task hierarchy and prioritize crew selection. */
.roster-crew-options label {
  grid-template-columns: 18px 82px minmax(80px, 1fr) minmax(150px, auto) 68px;
}
.roster-crew-options label.incompatible {
  cursor: not-allowed;
  opacity: .72;
}
.roster-crew-options label em { min-width: 0; justify-content: flex-end; text-align: right; }
...
@media (max-width: 699px) {
  ...
  .roster-crew-toolbar { grid-template-columns: minmax(0, 1fr) auto; gap: var(--sj-space-2); }
  .roster-crew-options label { grid-template-columns: 18px 82px minmax(0, 1fr) auto; }
  .roster-crew-options .technical-grade { grid-row: 2; grid-column: 3; text-align: left; }
  .roster-crew-options label em { grid-row: 1 / 3; grid-column: 4; }
}
</style>
