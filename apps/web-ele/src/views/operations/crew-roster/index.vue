...
const scheduleCanvasRef = ref<HTMLElement>();
...
function onScheduleWheel(event: WheelEvent) {
  if (!event.shiftKey || !scheduleCanvasRef.value) return;
  const delta = Math.abs(event.deltaY) >= Math.abs(event.deltaX)
    ? event.deltaY
    : event.deltaX;
  if (!delta) return;
  event.preventDefault();
  scheduleCanvasRef.value.scrollLeft += delta;
}
...
<template>
  <main :class="['crew-roster-page', 'sj-mission-control', { 'inspector-visible': hasSelectedFlight && !assignmentWorkspaceOpen }]">
    ...
    <section :class="['roster-workspace', { 'has-inspector': hasSelectedFlight }]">
      ...
      <section ref="scheduleCanvasRef" class="schedule-canvas" aria-label="机组执勤排班表" @wheel="onScheduleWheel">
...
<style scoped>
.crew-roster-page { --roster-filter-sticky-height: calc(var(--sj-control-dense) + var(--sj-space-6)); position: relative; min-height: 100%; color: var(--sj-text-1); background: var(--sj-canvas); font-family: var(--sj-font-ui); }.crew-roster-page.inspector-visible > :is(.filter-bar, .roster-workspace, .open-flight-strip) { margin-right: 332px; }
...
.assignment-workspace { display: grid; min-height: 100%; grid-template-rows: auto minmax(0, 1fr) auto; background: var(--sj-canvas); }
