  ...
  <main class="release-page sj-mission-control" data-starjet-theme="mission-control-dark">
    <header class="command-bar">
      <div class="filters">
        ...
        <label class="field registration-field"><span>注册号</span><select v-model="registrationFilter"><option>全部注册号</option><option v-for="item in registrationOptions" :key="item">{{ item }}</option></select></label>
        <label class="field release-status-field"><span>放行状态</span><select v-model="statusFilter"><option>全部状态</option><option>已放行</option><option>待放行</option><option>准备中</option><option>受阻</option></select></label>
      </div>
...
.command-bar { height: auto; flex-wrap: wrap; }
.filters { overflow: visible; flex: 1 1 920px; flex-wrap: wrap; }
.filters .field { flex: 1 1 126px; }
.filters .registration-field { min-width: 118px; flex: 0 0 118px; }
.filters .release-status-field { min-width: 108px; flex: 0 0 108px; }
.command-bar > .secondary-button { margin-left: auto; flex: 0 0 auto; }
@media (max-width: 1280px) {
  .command-bar { align-items: flex-end; flex-direction: row; }
  .filters { flex-basis: 100%; }
  .command-bar > .secondary-button { align-self: auto; }
}
</style>
